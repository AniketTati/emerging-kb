...
for _ in $(seq 1 60); do
    ...
    if [[ "$s" == "parsed" || "$s" == "chunked" || "$s" == "contextualized" || "$s" == "embedded" || "$s" == "raptor_building" || "$s" == "mentions_extracting" || "$s" == "fields_extracting" || "$s" == "units_extracting" || "$s" == "entities_extracting" || "$s" == "identity_resolving" || "$s" == "ready" ]]; then parsed=1; break; fi
