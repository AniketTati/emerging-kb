...
for _ in $(seq 1 180); do
    ...
    # Phase 3d chained raptor_build_file may race past 'embedded' to
    # 'raptor_building'/'ready'. Any post-embedded state counts as
    ...
    if [[ "$s" == "embedded" || "$s" == "raptor_building" || "$s" == "mentions_extracting" || "$s" == "fields_extracting" || "$s" == "units_extracting" || "$s" == "entities_extracting" || "$s" == "identity_resolving" || "$s" == "ready" ]]; then embedded=1; break; fi
