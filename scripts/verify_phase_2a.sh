...
for _ in $(seq 1 120); do
    ...
    if [[ "$state" == "parsed" || "$state" == "chunked" || "$state" == "contextualized" || "$state" == "embedded" || "$state" == "raptor_building" || "$state" == "mentions_extracting" || "$state" == "fields_extracting" || "$state" == "units_extracting" || "$state" == "entities_extracting" || "$state" == "identity_resolving" || "$state" == "ready" ]]; then parsed=1; break; fi
