...
for _ in $(seq 1 180); do
    ...
    if [[ "$s" == "contextualized" || "$s" == "embedded" || "$s" == "raptor_building" || "$s" == "mentions_extracting" || "$s" == "fields_extracting" || "$s" == "units_extracting" || "$s" == "entities_extracting" || "$s" == "identity_resolving" || "$s" == "ready" ]]; then contextualized=1; break; fi
