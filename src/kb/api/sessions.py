...
async def get_session_turns(
    ...
    if s is None:
        raise HTTPException(status_code=404, detail="session not found")
    # DISPLAY path — return ALL turns in chronological order. The user
    # rightly insisted the display side is unbounded; capping it was the
    # original bug. There is no `limit` query param because there isn't
    # a sensible UX where the chat UI would want to render only the
    # first / last N — when a session grows that big, the UI scrolls,
    # it doesn't truncate.
    #
    # ANSWER-TIME path is separate and intentionally capped: the
    # orchestrator calls read_last_k_turns(k=DEFAULT_HOT_TURNS) to pick
    # the verbatim turns the LLM sees + asks turn_summarizer to roll
    # older turns into older_turn_summary. That 6-turn cap is the
    # context-window budget, NOT a display cap. Conflating the two was
    # how we shipped "only first N chats survive a reload".
    #
    # LEFT JOIN to query_log so the chat UI replay shows mode / intent /
    # crag / faithfulness inline instead of "?" — those columns live on
    # query_log, not chat_turns.
