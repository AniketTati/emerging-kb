...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "'email_message', 'row', etc.). To query inside a collection:\n"
        "  - 'sum of transactions' / 'count transactions' → use\n"
        "    extracted_entities, filter unit_type='transaction'.\n"
        "  - 'sum the debits in bank statements' → filter\n"
        "    unit_type='transaction'; the per-row money value lives in\n"
        "    the `fields` jsonb column under keys like 'debit' or\n"
        "    'amount'. Q-mode CANNOT extract values out of jsonb today,\n"
        "    so for jsonb-keyed sums issue a refuse with a clear\n"
        "    reason (e.g. 'aggregating jsonb fields like debit not\n"
        "    supported yet — query the count instead').\n"
        "  - 'count of line items per invoice' → filter\n"
        "    unit_type='line_item' + group_by=['file_id'].\n"
        "  - 'invoices with > 10 line items' → not expressible without\n"
        "    HAVING; refuse with reason.\n"
        "  - Use `extracted_entities.rarity_score` to find anomalies\n"
        "    (high rarity = unusual unit).\n"
        "\n"
