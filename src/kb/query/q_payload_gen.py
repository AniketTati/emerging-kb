...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "with `value_text` (always TEXT) and `inferred_doc_type` directly\n"
        "on the row (no join needed).\n"
        "\n"
        "PREFER `value_numeric` (a real numeric column) over\n"
        "`value_text::numeric` for SUM / AVG / MIN / MAX. It's pre-parsed\n"
        "from value_text (handles '₹22 lakh' / '5,300,000' / '$5.3M' /\n"
        "'(500)' / 'INR 1.28 cr' / Indian comma grouping all correctly)\n"
        "and is NULL for non-numeric values (which the aggregation\n"
        "correctly skips). `value_text::numeric` only works on bare\n"
        "numeric strings and silently NULLs rows formatted with\n"
        "currency symbols or magnitude words.\n"
        "Currency tag lives in `value_currency` (ISO 4217 string).\n"
        "\n"
        ...
        "              {field:'field_name',op:'eq',value:'total_cost_inr'}],\n"
        "     aggregations:[{op:'SUM', field:'value_numeric', alias:'total'}]}\n"
        ...
        "\n"
        "## Days between two events — emit MIN + MAX over dates and let\n"
        "## the answerer subtract\n"
        "When the question asks for a DURATION or DAYS BETWEEN two\n"
        "named events (e.g. 'duration from contract signing to mechanical\n"
        "completion', 'days between RFI raised and response'), emit a\n"
        "plan that returns BOTH date endpoints as separate aggregations.\n"
        "The downstream answerer computes the difference and reports the\n"
        "number of days. DO NOT refuse just because 'subtraction isn't\n"
        "supported' — your job is to surface the two dates; the generator\n"
        "subtracts.\n"
        "Examples:\n"
        "  - 'total project duration in days from contract signing to\n"
        "     mechanical completion' →\n"
        "    {from:'proposed_fields',\n"
        "     filters:[{field:'field_name',op:'in',\n"
        "              value:['contract_effective_date','mechanical_completion_date',\n"
        "                     'contract_signing_date','contract_date']}],\n"
        "     aggregations:[{op:'MIN', field:'value_text::date', alias:'start_date'},\n"
        "                   {op:'MAX', field:'value_text::date', alias:'end_date'}]}\n"
        "  - 'days between RFI raised and response' →\n"
        "    {from:'proposed_fields',\n"
        "     filters:[{field:'inferred_doc_type',op:'eq',value:'rfi_response'},\n"
        "              {field:'field_name',op:'in',\n"
        "              value:['rfi_date','response_date']}],\n"
        "     aggregations:[{op:'MIN',field:'value_text::date',alias:'rfi_open'},\n"
        "                   {op:'MAX',field:'value_text::date',alias:'rfi_close'}]}\n"
        "\n"
        "## Peak / max over time — use MAX on the numeric field\n"
        "When the question asks for PEAK, HIGHEST, MAX, MOST X (e.g.\n"
        "'peak headcount', 'highest temperature', 'busiest day') over a\n"
        "time-series of documents, this IS expressible as MAX on the\n"
        "underlying numeric field. DO NOT refuse with 'max over time not\n"
        "supported' — MAX is supported. Pair with AVG when both peak and\n"
        "average are useful.\n"
        "Examples:\n"
        "  - 'average headcount at peak on the site' →\n"
        "    {from:'proposed_fields',\n"
        "     filters:[{field:'inferred_doc_type',op:'eq',value:'daily_site_report'},\n"
        "              {field:'field_name',op:'eq',value:'total_headcount'}],\n"
        "     aggregations:[{op:'MAX', field:'value_numeric', alias:'peak'},\n"
        "                   {op:'AVG', field:'value_numeric', alias:'avg_headcount'}]}\n"
        "  - 'highest daily temperature recorded' →\n"
        "    {from:'proposed_fields',\n"
        "     filters:[{field:'inferred_doc_type',op:'eq',value:'daily_site_report'},\n"
        "              {field:'field_name',op:'eq',value:'temperature_celsius'}],\n"
        "     aggregations:[{op:'MAX', field:'value_numeric', alias:'max_temp'}]}\n"
        "\n"
