...
**Now:** 🎉 **Wave A end-to-end stack shipped + KV+Tables collapse + hierarchical chunking landed.** Ingestion (Phases 2a–7), retrieval (Phases 8a–8h: 6-channel + RRF + Cohere rerank + CRAG + Astute + faithfulness + 13 planner modes), SSE audit (Phase 9), and **9 of 10 UI surfaces** (`/chat`, `/upload`, `/explore` + `/explore/entity/[id]`, `/schema-studio`, `/dashboard`, `/files/[id]`, `/audit`, `/settings`, `/playground`) all ✅ on `main` through PR #48 + 66c0fff. `/extraction-studio` ships as a Wave-C roadmap page. Phase 11 demo-corpus loader + Phase 12 eval runner + RAGAS/HHEM scoring + `POST /eval/run` (PR #41) all ✅. All 9 tier-1 gap designs from `gaps_design.md` ship as code. Post-Wave-A work: PR #42 (KV+Tables collapse + Q-mode jsonb), PRs #43–#44 (13 planner modes + S/T mode fixes), PR #45 (`entities` → `canonical_entities` + `atomic_units` drop), PR #46 (LlamaIndex `HierarchicalNodeParser` + AutoMergingRetriever + `chunker_configs` runtime routing), PR #47 (`scripts/wipe_and_reprocess.sh`), PR #48 (children-survive-schema_entities guard + first `singularize` pass), `66c0fff` (singularize `es`-suffix hole closed + 27-case unit-test matrix). Full 26-doc demo corpus reprocessed clean through the new pipeline: 26/26 `ready`, 253 sub-entity children across 58 distinct singular `unit_type`s, 251 canonical_entities, 1,249 graph edges. Hash-chained `audit_log` + integrity walker live.
**Next:** **Step 5 UI polish** — surface the new chunking concepts in the UI: tree-view in Doc Detail chunks panel (using `parent_chunk_id` + `node_level`), Plan Inspector "X leaves → Y parents via auto-merge", chunker_configs admin view. After that: Wave B competitive commitments (B1 batch query, B2 `deep_research` agentic loop, B3 DSPy prompt optimization, B4 multi-agent Q-mode decomposition) + Phase 13 NotebookLM-style artifacts + Phase 14 HippoRAG graph. See `docs/competitive_audit.md` §5 for the 4 Wave B commitments.
**Blocked on:** nothing.
...
| Date | Change | By |
| 2026-05-27 | **`singularize_unit_type()` hardened + 27-case test matrix (`66c0fff`).** Post-#48 corpus reprocess surfaced 8 unit_types that escaped singularization because the prior rule returned every "es"-ending name unchanged — masking the "ies → y" rule (`responsibilities` never reached it) and ignoring legitimate plurals (`services`, `expenses`, `quotes`, `attendees`, `considered_alternatives`, `work_experiences`, `diagnoses`). Reordered the rules so "ies" runs first; tightened the "don't touch this" early-return to actually-singular Latin/Greek endings (ss/is/us/as); added an explicit set for English add-`es` plurals (`sses`/`xes`/`zzes`/`shes`/`ches`) that drop `es` instead of just `s` (note: `zes` alone is excluded — `sizes`/`breezes`/`phrases` have a silent `e` in the singular root). Hoisted the helper from a nested function inside `extract_kv_tables_file_impl` to module scope as `singularize_unit_type` so the edge-case matrix can be unit-tested directly. New `test_singularize_unit_type` `pytest.parametrize` covers 27 cases — all 8 previously-slipping unit_types pin to their correct singular form. In-place `UPDATE extracted_entities` patched the 35 already-stored rows. | Aniket |
| 2026-05-27 | **PR #48: 2 regressions surfaced by post-PR-#46 wipe+reprocess (`40c4e10`).** Both surfaced when the first corpus reprocess through the new hierarchical chunker landed children that then disappeared and unit_types that didn't match Q-mode prompt examples. **Fix #1 — children survive `schema_entities` phase**: `extract_schema_entities_file_impl` previously started its transaction with `delete_extracted_entities_for_file()`, which wiped the typed sub-entity children that `extract_kv_tables_file_impl` had just written upstream in the same pipeline run. Added `delete_extracted_entities_parents_for_file` (DELETE … WHERE `unit_type IS NULL`) and rewired the worker to use it. The new helper deletes only doc_root rows; the children survive into the lineage pass. **Fix #2 — initial `_singularize()` pass**: the LLM tended to emit plural table names (`transactions`, `clauses`, `line_items`); Q-mode prompt examples + retrieval-channel keyword filters expect singular. Added a nested `_singularize()` helper at the storage boundary in `extract_kv_tables_file_impl` so `unit_type` is canonicalized to singular form before INSERT. (Hole in this initial pass closed in `66c0fff` above.) | Aniket |
| 2026-05-27 | **PR #47: `scripts/wipe_and_reprocess.sh` (`b654a96`).** Operational script for rebuilding pipeline state after schema or chunker changes. TRUNCATEs downstream tables (chunks, canonical_entities, extracted_entities, auto-promoted schema_fields, auto schemas, proposed_fields, inferred_schema_fields) while keeping `raw_pages` intact (no re-parse needed). Rolls every non-deleted file back to `lifecycle_state='parsed'` so `chunk_file_impl` picks them up. Uses a small Python heredoc with `procrastinate_app.open_async()` to enqueue chunk_file deferrals for the rolled-back files. Worker processes them serially at concurrency=1; chain runs `chunk → contextualize → embed → raptor → mentions → kv_tables → schema_entities → identities → triples → graph → ready` per file. | Aniket |
| 2026-05-27 | **PR #46: LlamaIndex `HierarchicalNodeParser` + AutoMergingRetriever + runtime chunker config (`f3bc5b8`).** Replaces the custom token-bounded chunker with `HierarchicalNodeParser` from LlamaIndex (sizes `[2048, 512, 128]` — three node levels). Migration 0040 adds `chunks.node_level smallint NOT NULL DEFAULT 0` + `chunks.parent_chunk_id uuid REFERENCES chunks(id)` self-FK so parent ↔ leaf relationships persist. New `chunker_configs` runtime table routes by `(workspace_id, doc_type) → chunker_kind` without code deploys — 3 kinds ship: `hierarchical` (default — prose/contracts/reports), `row_per_leaf` (xlsx/tabular — one leaf per row), `message_per_leaf` (email threads — one leaf per message). `chunk_file_impl` rewritten to use `kb.chunking.doc_type_router.select_chunker()` + `run_chunker()` + topological-order INSERT so parent rows always exist when their children's FK lands. Retrieval-time `AutoMergingRetriever` (`kb/query/auto_merging.py`) detects when ≥ `merge_threshold` (default 0.5) of a parent's leaves appear in the same hit list and swaps them for a synthesized parent hit — surfaces the right granularity per query without re-chunking. RAPTOR + embeddings still operate on the `node_level=0` leaves; everything upstream of step 6 in the pipeline diagram is unchanged. **Verified end-to-end against all 3 chunker_kinds**: bank-statement.xlsx (`row_per_leaf`, 14 transaction children, Q-mode sum-debits → $452,998.10), vertex-msa.pdf (`hierarchical`, 3 `service_location` children), it-incident-thread.eml (`message_per_leaf`, 5 children across 2 typed sub_entities). | Aniket |
| 2026-05-27 | **PR #45: rename `entities → canonical_entities` + drop `atomic_units` everywhere (`8d095f8`).** Cleanup PR to align the storage layer with the post-KV+Tables collapse model. The old `atomic_units` staging table is gone — `extract_kv_tables_file_impl` already writes the canonical storage shape directly to `extracted_entities` (with `unit_type` set + `parent_entity_id` left NULL for the lineage pass), so the intermediate table was pure duplication. Companion rename: `entities` → `canonical_entities` to disambiguate from `extracted_entities` (the latter being typed instances per-doc, the former being the workspace-wide identity-resolved directory). Migration 0042 handles both: drops `atomic_units` + its indexes/RLS/grants, renames `entities` table + its FKs everywhere they're referenced (graph_edges, mention_to_entity, entity_aliases, identity_resolutions). Every domain function + worker + API reference updated. PASS 1.5 of `extract_schema_entities_file_impl` (the "promote atomic_units to children" step) deleted as dead code — its work moved upstream into the KV+Tables worker by PR #42. | Aniket |
| 2026-05-27 | **KV+Tables collapse + Q-mode jsonb support shipped on `feat/mode-completion`.** Two intertwined commits close the loop on the original "sum transactions" refusal that motivated this whole refactor. **(1) Extraction collapse** — `src/kb/extraction/kv_tables.py` (NEW, ~520 LOC) provides a single structured-output LLM call returning `{doc_type, scalars[], tables[]}` with per-row `source_chunk` / `source_char_*`. Replaces the legacy 3-call sequence (L2b classify + L2b propose + L3 plugin) inside a new `extract_kv_tables_file_impl` task in `src/kb/workers/tasks.py`. Scalars fan out to the existing `proposed_fields → cluster → inferred_schema_fields → promote → schema_fields` pipeline; table rows fan out to `atomic_units` keyed by `unit_type=<table.name>`. Lifecycle jumps `fields_extracting → entities_extracting`, skipping the legacy `units_extracting` slot. `extract_mentions_file_impl` rewired to defer the new task; `/files/:id/re-extract` follows. Phase 1.5 nested-entity bootstrap inside `extract_schema_entities_file_impl` translates `chunks.id → contextual_chunks.id` so `atomic_units.source_chunk_id` (FK → chunks) and `extracted_entities.source_chunk_id` (FK → contextual_chunks) both validate. Per-doc Gemini calls: 5 → 2 (mentions + KV+Tables); identical downstream tables and schemas. Legacy `extract_fields_file_impl` + `extract_atomic_units_file_impl` retained as dead code for rollback. **(2) Q-mode jsonb aggregations + filters** — `Aggregation.field` + `Filter.field` accept `<col>.<key>::<cast>` syntax (e.g. `fields.debit::numeric`) in addition to plain identifiers; grammar parses into `jsonb_path: (col, key, cast)`; catalog adds `JSONB_AGG_ALLOWED = {('extracted_entities','fields')}`; validator enforces (table, jsonb_col) allowlist + cast-vs-op type matrix (SUM/AVG→numeric casts; MIN/MAX→numeric|text|date|timestamptz; like→text); compiler emits `(t."fields"->>'key')::cast` via a single `_jsonb_extract_sql()` helper with key quote-escaping. q_payload_gen prompt rewritten to teach the LLM the new syntax with 5 worked examples instead of refusing. **Tests**: 25 new `test_kv_tables_unit.py` (parser + truncation recovery + Gemini fake-client + factory matrix) + 3 new `test_kv_tables_worker.py` (identity + happy-path + state guard) + 10 new `test_b4b_unit.py` (jsonb parse/validate/compile + cast-vs-op + injection attempt). 75 Q-mode tests + 111 extraction tests green. **Verified end-to-end against the live demo workspace**: 4 doc types re-extracted (bank_statement 9 scalars + 14 transactions · MSA 22 scalars + 0 tables · email_thread 15 scalars + 2 tables × 5 rows · lab_report 12 scalars + 1 table × 15 rows). Q-mode "sum all debits" → 452998.10; "largest debit transaction in March 2026" → 196220.45. Bank_statement schema tree: BankStatement (doc_root) → Transaction (sub_entity). Branch ahead of origin by 5 commits (`7e188df` kv_tables extractor · `7ef9212` worker wiring · `65d4c1f` FK translation fix · `58fcff8` jsonb aggregations · `1b74492` jsonb filters). | Aniket |
|---|---|---|
