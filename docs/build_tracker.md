...
**Now:** Phase 3d G1 ✅ + G2 ✅ + G3 ✅ + G4 🟡 OPEN — RAPTOR tree build, per-doc. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3d G4 (0012 migration + `kb/raptor/` + `kb/summarization/` + `kb/domain/raptor.py` + worker `raptor_build_file_impl` + chained defer from `embed_file_impl`). Then G5 (verify_phase_3d.sh + 11-script cross-phase sweep), then Phase 3e (corpus-level, ~4-6 hr, reuses 3d tables via the forward-compat columns).
**Blocked on:** nothing.
...
| **3d** | RAPTOR tree build, **per-doc** — recursive cluster→summarize→re-embed → `raptor_nodes` (L2..6) + `raptor_edges` (discriminated child FK); intermediate lifecycle state `raptor_building` between `embedded` → `ready` | ✅ | ✅ | ✅ | 🟡 | ⬜ | G3 ✅ 2026-05-24: spec at `tests/specs/phase_3d.md` + 17 red (6 raptor unit + 6 summarization unit + 5 worker integration). Mock `_MockGeminiClient` mirrors 3b-bis pattern. Suite: 258/258 + 17 RED. G4 opens. |
...
### 5.10 Phase 3d plan — RAPTOR tree build, per-doc (G1 ✅ + G2 ✅ + G3 ✅ + G4 🟡 OPEN)
