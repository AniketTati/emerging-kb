...
**Next:** **Phase 4 G4 opens** (build). G1 ✅ + G3 ✅ signed off — 16 decisions locked at §5.11; spec at `tests/specs/phase_4.md`; 13 red tests committed (12 RED + 1 regression-guard pre-passing for decision #15). G4 deliverable: `migrations/sql/0013_indexes.sql` (4 indexes — HNSW on `chunk_embeddings.embedding` + `raptor_nodes.embedding`; BM25 on `contextual_chunks.contextual_text` + `raptor_nodes.text`) + `src/kb/retrieval/{__init__,smoke}.py` (internal smoke helper, NOT mounted on any router) + `scripts/reindex_weekly.sh` (cron stub). Estimated wall-clock: 2-3 hr. G5 follows immediately: `verify_phase_4.sh` + cross-phase sweep across 13 verify scripts.
**Blocked on:** nothing.
...
| **4** | Indexing: pgvector HNSW + pg_search BM25 on all RAPTOR levels | ✅ | — | ✅ | ⬜ | ⬜ | G1 ✅ + G3 ✅ signed off 2026-05-25 (§5.11, 16 decisions). G2 was a no-op per decision #16. G3 lands 13 tests (12 RED + 1 regression-guard pre-passing) across `test_indexes.py` + `test_retrieval_smoke.py`; 286/286 existing tests still green. Branch `phase-4/retrieval`. |
