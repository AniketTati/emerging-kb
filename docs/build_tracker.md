...
**Now:** Phase 3d G1 ✅ + G2 ✅ + G3 🟡 OPEN — RAPTOR tree build, per-doc. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3d G3 (spec + ~16 red skeletons) → G4 (0012 migration + raptor + summarization + worker) → G5 (verify_phase_3d.sh + 11-script cross-phase sweep). Then Phase 3e at §5.10.1 (corpus-level, ~4-6 hr, reuses 3d tables via the forward-compat columns).
**Blocked on:** nothing.
...
| **3d** | RAPTOR tree build, **per-doc** — recursive cluster→summarize→re-embed → `raptor_nodes` (L2..6) + `raptor_edges` (discriminated child FK); intermediate lifecycle state `raptor_building` between `embedded` → `ready` | ✅ | ✅ | 🟡 | ⬜ | ⬜ | G2 ✅ 2026-05-24: api_contracts §5.2 lifecycle enum widened with `raptor_building` + `ready` framed as 3d's terminal + §5.3 lifecycle history example annotated with all post-2c stage transitions + event names + payload shapes. G3 opens: spec + ~16 red skeletons. |
...
### 5.10 Phase 3d plan — RAPTOR tree build, per-doc (G1 ✅ + G2 ✅ + G3 🟡 OPEN)
