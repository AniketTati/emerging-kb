...
**Next:** **Phase 8e ✅ FULLY GREEN** — Astute generation shipped 2026-05-25 on `phase-8e/generate` (stacked on `phase-8d/crag`). **491/491 pytest in 98s** · cross-phase sweep across all 21 verify scripts: 21/21 GREEN in 9:31. Module `src/kb/query/generate.py` (~330 LOC) with `Generator` Protocol + `GenerationResult` + `Citation` Pydantic models + 2-impl factory (`GeminiGenerator` + `IdentityGenerator`) per Astute RAG paper (Wang et al. 2024). Cite-or-refuse defensive prompt over reranked top-10 hits. 15 decisions locked at §5.15.5 including #14 anthropic→Identity Wave-A defer and #10 LLM-error-→refusal (asymmetric vs CRAG which fail-safe-passes — fake-answer consequence >> refusal consequence). 19 new unit tests (Pydantic shapes + parser fail-safes + Identity templated echo + Gemini path with top-10 capping + force_refuse skips LLM + system-instruction Astute prompt asserted). No migration; no API contract change. **Phase 8f opens next** — orchestrator stitches 8a→8e + `POST /search` + `POST /chat` + `query_log` migration (the HTTP surface this whole module set has been building toward). Then 9 (SSE + audit), 10a/10b (Next.js UI). Stacked-branch PR strategy: each phase off the previous unmerged phase.
**Blocked on:** nothing.
...
| **8f** | HTTP surface — `POST /search` + `POST /chat` + `query_log` audit table | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | G1 DRAFT at §5.15.6. Orchestrator stitches 8a→8e. |
