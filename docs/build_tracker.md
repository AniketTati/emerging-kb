...
| **3a** | Chunking — late chunking of `raw_pages` → `chunks` table (layout-aware, token-bounded, cross-page joining); worker stage `chunk_file`; new lifecycle state `chunked` | ✅ | ✅ | ✅ | ✅ | ⬜ | Internal worker; no LLM/embedding calls. 16/16 new tests green first run; suite 204/204 |
...
### 5.7 Phase 3a plan — Chunking (G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ SIGNED OFF)

> **Status:** G1–G4 all green 2026-05-23. Plan + contract delta + 16 red skeletons + working implementation locked. Branch: `phase-3/chunking-raptor` off `main` (Phase 2b merged as PR #6 · merge commit `971a019`). 204/204 pytest green; zero in-G4 fixes.
