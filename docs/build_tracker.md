...
**Now:** Phase 2c G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN. Code landed: 4 new modules (text_layer_sniff, quality, gemini_ocr_parser + dispatcher widening + worker escalation + API ?parser= override + .env.example). 258/258 pytest in 58s. G5: new verify_phase_2c.sh with Docker-stack E2E for the scanned-PDF auto-route → Gemini OCR path (gated on KB_GEMINI_API_KEY) + Identity-fallback path + caller override + invalid-parser-override 400 + cross-phase sweep.
**Next:** Phase 2c G5 → close 2c → Phase 3d (RAPTOR).
**Blocked on:** nothing.
...
| **2c** | Gemini OCR + strategy-driven dispatch — `GeminiOCRParser` (pypdfium2 PDF→PNG + Gemini 2.5 Flash VLM, per-page) + pre-flight text-layer sniff + 4-value `KB_PARSER_STRATEGY ∈ {auto,docling_first,gemini_first,gemini_only}` + caller override `?parser=...` + quality escalation + provenance JSON in `raw_pages.layout_json`. | ✅ | ✅ | ✅ | ✅ | 🟡 | G4 ✅: 4 new modules (text_layer_sniff.py 70 LOC, quality.py 130 LOC, gemini_ocr_parser.py 170 LOC, tiny_scanned.pdf fixture + generator script). Dispatcher widened with `select_parser_for()`. Worker `parse_file_impl` accepts `forced_parser`, wires escalation via `_maybe_escalate_to_ocr`. API accepts `?parser=`, persists into upload lifecycle event. `.env.example` documents 6 new env vars. One in-G4 refinement: `auto`/`gemini_first` soft-fall-back to Docling when no Gemini key (strict-fail reserved for `gemini_only` + explicit `?parser=gemini`). Full suite: 258/258 in 58s. G5: new verify_phase_2c.sh + cross-phase sweep. |
...
### 5.6.1 Phase 2c plan — Gemini OCR + strategy-driven parser dispatch (G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN)
