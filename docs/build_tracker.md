...
**Now:** Phase 3e G1 ✅ + G2 ✅ + G3 ✅ + G4 🟡 OPEN — corpus-level RAPTOR. G3 RED: 11 new tests (4 corpus unit + 4 corpus worker integration + 3 API). Coverage hits all 15 G1 decisions. Suite: 275/275 + 11 RED (no collateral damage). G4 opens: `umap-learn` dep + `kb/raptor/corpus.py` + `raptor_build_corpus_impl` worker + `kb/api/corpus.py` router + main.py mount + 2 new error types. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3e G4 (code: `umap-learn` dep + `kb/raptor/corpus.py` + `raptor_build_corpus_impl` worker + `raptor_build_corpus` Procrastinate task + `kb/api/corpus.py` router + new error types `CorpusRebuildNoInputError` / `CorpusRebuildInFlightError` + mount in main.py) → G5 (verify_phase_3e.sh + 12-script cross-phase sweep). After 3e, Wave A ingestion + tree-building is fully complete; retrieval (Phase 4 HNSW + BM25 + tree-aware query) opens.
**Blocked on:** nothing.
...
| **3e** | RAPTOR tree build, **corpus-level** — cluster doc-roots across workspace → summarize themes → write `scope='corpus'` rows. Explicit `POST /corpus/raptor/rebuild` trigger (not auto). UMAP+GMM swap-in for the N=100K case where AC is infeasible. | ✅ | ✅ | ✅ | 🟡 | ⬜ | G3 ✅ 2026-05-24: spec at `tests/specs/phase_3e.md` + 11 red (4 corpus unit + 4 corpus worker integration + 3 API tests). Suite: 275/275 + 11 RED. G4 opens. |
...
### 5.10.1 Phase 3e plan — RAPTOR tree build, corpus-level (G1 ✅ + G2 ✅ + G3 ✅ + G4 🟡 OPEN)
