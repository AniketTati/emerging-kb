...
| **3c** | Embedding — Gemini Embedding 001 on contextual chunks → `chunk_embeddings` (`halfvec(3072)`); worker stage `embed_file`; new lifecycle state `embedded` | ✅ | ✅ | ✅ | ⬜ | ⬜ | First embedding call; gated on `KB_GEMINI_API_KEY` with DeterministicMockEmbedder for CI; HNSW index lands in Phase 4. 13 new red skeletons (7 unit + 6 worker); suite collect 219 → 232 |
...
### 5.9 Phase 3c plan — Embedding (G1 ✅ + G2 ✅ + G3 ✅ SIGNED OFF)

> **Status:** G1+G2+G3 signed off 2026-05-23. Plan + single contract delta + 13 red skeletons (7 unit + 6 worker) locked. Branch: `phase-3/chunking-raptor` (third commit-set). Phase 3 originally listed RAPTOR + embeddings as one piece; split into 3c (embedding) + 3d (RAPTOR build).
