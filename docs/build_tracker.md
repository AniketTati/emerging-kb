...
**Now:** Phase 3b-bis G1 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN. `GeminiContextualizer` lands (~110 LOC, mirrors `AnthropicContextualizer` shape via google-genai SDK). Factory widened to 4-value `KB_CONTEXTUALIZER` selector. Full suite 238/238. G5 next: extend `verify_phase_3b.sh` with a Gemini-path E2E branch (gated on `KB_GEMINI_API_KEY` propagation) + cross-phase sweep.
**Next:** Phase 3b-bis G5 → close 3b-bis → Phase 3d (RAPTOR). Phase 1c-bis (GeminiOCRParser) gated on demo-corpus answer.
...
| **3b-bis** | Gemini Contextualizer adapter — `GeminiContextualizer` alongside `AnthropicContextualizer` + factory selector `KB_CONTEXTUALIZER ∈ {gemini,anthropic,identity,auto}`. No schema/lifecycle/API delta. | ✅ | — | ✅ | ✅ | 🟡 | G4 ✅: `GeminiContextualizer` (~110 LOC) + widened `make_contextualizer()` factory (auto-probe Gemini → Anthropic → Identity). Full suite: 238/238 (232 prior + 6 new). One in-G4 fix: G3's `thinking_budget` test used `or` on getattr which short-circuits on 0; refactored to explicit `hasattr` branches. G5: extend verify_phase_3b.sh with Gemini-path E2E branch + cross-phase sweep. |
...
### 5.8.1 Phase 3b-bis plan — Gemini Contextualizer adapter (G1 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN)
