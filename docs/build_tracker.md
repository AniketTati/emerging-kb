...
**Now:** Phase 2b G4 — building (`kb/parsers/{xlsx,email,mistral_ocr}_parser.py` + extend `register_default_parsers()` + widen `_PHASE_2A_WHITELIST` + magic-byte sniffer in `_handle_multipart` + commit `tiny.xlsx` + `tiny.eml` + `tiny_with_attachment.eml` fixtures). Branch: `phase-2b/parse-formats`.
**Next:** Phase 2b G5 — `verify_phase_2b.sh` + cross-phase sweep + PR.
**Blocked on:** nothing.
...
| **2b** | Parse layer — **additional parsers**: xlsx (openpyxl) + email (stdlib) + Mistral OCR (external API adapter class + mock-tested; real-API gated on `KB_MISTRAL_API_KEY`) | ✅ | ✅ | ✅ | 🟡 | ⬜ | G1+G2+G3 ✅ signed off 2026-05-23. 18 new tests collected (xlsx 5 + email 5 + mistral_ocr 5 + files_crud 3 additive); pytest --collect-only confirms 188 total. G4 building now. |
