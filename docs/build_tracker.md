...
**Now:** Phase 3d ✅ shipped — per-doc RAPTOR tree build. 275/275 pytest. verify_phase_3d.sh 22/22. Cross-phase sweep across all 11 verify scripts (0/1a/1b/1c/2a/2b/2c/3a/3b/3c/3d) all GREEN — **192 checks total**. Wave A ingestion side is complete; `files.lifecycle_state='ready'` is reachable end-to-end. Branch `phase-3/chunking-raptor` now carries 6 commit-sets (3a/3b/3c/3b-bis/2c/3d) ready to merge or extend with 3e. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3e (corpus-level RAPTOR, ~4-6 hr) — reuses 3d tables via the forward-compat columns; switches to UMAP+GMM for N=100K scale; adds `POST /corpus/raptor/rebuild` endpoint. After 3e, Wave A retrieval (Phase 4) opens.
**Blocked on:** nothing.
