...
**Now:** Phase 3d G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN — RAPTOR tree build, per-doc. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3d G5 (new `scripts/verify_phase_3d.sh` + 11-script cross-phase sweep) → push. Then Phase 3e (corpus-level, ~4-6 hr, reuses 3d tables via the forward-compat columns).
**Blocked on:** nothing.
...
| **3d** | RAPTOR tree build, **per-doc** — recursive cluster→summarize→re-embed → `raptor_nodes` (L2..6) + `raptor_edges` (discriminated child FK); intermediate lifecycle state `raptor_building` between `embedded` → `ready` | ✅ | ✅ | ✅ | ✅ | 🟡 | G4 ✅ 2026-05-24: 5 new modules + 2 mutations. `migrations/sql/0012_raptor.sql` (raptor_nodes + raptor_edges + lifecycle CHECK widens with `raptor_building`). `src/kb/summarization/__init__.py` (Summarizer Protocol + GeminiSummarizer + AnthropicSummarizer + IdentitySummarizer + 4-value KB_SUMMARIZER factory). `src/kb/raptor/__init__.py` (AC-cosine `cluster_embeddings` + `_build_in_memory` test-injectable orchestrator). `src/kb/domain/raptor.py` (RaptorNode + insert helpers + discriminated edge insert + leaf reader). `src/kb/workers/tasks.py` MUTATED: `raptor_build_file_impl` (cluster→summarize→embed→atomic DB write→lifecycle transition) + chained defer from embed_file_impl + raptor_build_file Procrastinate task. `.env.example` adds KB_SUMMARIZER + commented overrides. Three in-G4 fixes: (1) raptor_edges PRIMARY KEY (parent, child_node_id, child_chunk_id) made columns implicitly NOT NULL — replaced with synthetic `id` PK; (2) worker tests originally drove tiny.pdf through real pipeline → only 1 chunk → no L2 nodes; refactored to seed N=5 fabricated leaves via direct SQL; (3) test_raptor_build_writes_raptor_build_done + test_raptor_build_failure_writes_failed missing db_url_superuser fixture in signatures. Suite: 275/275 in 66s. G5 opens. |
| **3e** | RAPTOR tree build, **corpus-level** — cluster doc-roots across workspace → summarize themes → write `scope='corpus'` rows. Explicit `POST /corpus/raptor/rebuild` trigger (not auto). UMAP+GMM swap-in for the N=100K case where AC is infeasible. | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Plan sketch at §5.10.1. Reuses 3d's tables (scope='corpus', file_id NULL — locked at 3d G4). New deps: umap-learn + pynndescent. Open in Wave A (admin gating → Phase 9). Opens after 3d G5. |
...
### 5.10 Phase 3d plan — RAPTOR tree build, per-doc (G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN)
...
### 5.10.1 Phase 3e plan — RAPTOR tree build, corpus-level (placeholder)

> **Status:** ⬜ Not yet drafted. Opens after Phase 3d G5 ✅. Same `phase-3/chunking-raptor` branch (7th commit-set after 3d). Final Wave A phase; closes the architecture's "RAPTOR builds the hierarchy of what's there" (line 41) promise at corpus scale.

#### Scope sketch (to be locked at G1 when 3d closes)

Corpus-level RAPTOR tree across all per-doc roots in a workspace, written into the same `raptor_nodes` + `raptor_edges` tables with `scope='corpus'` and `file_id=NULL`. Forward-compat columns landed at 3d's 0012 migration; no schema delta at 3e.

**In scope:**
- **`kb/raptor/corpus.py`** — `cluster_embeddings_corpus(vectors)` switches algorithm from AC (per-doc) to **UMAP + sklearn GaussianMixture** per RAPTOR paper. UMAP reduces 3072 → ~10 dim; GMM soft-clusters in low-dim. Required because AgglomerativeClustering is O(N²) = 10^10 distance computations at N=100K corpus roots, infeasible. Soft clustering matters more at corpus scale (a doc spans multiple themes).
- **`build_corpus_tree(workspace_id)`** — reads all per-doc roots (`raptor_nodes WHERE scope='per_doc' AND level = max_level_for_file(file_id)`) → clusters → summarize → embed → write `scope='corpus'` rows. Same `Summarizer` + `Embedder` adapter pattern as 3d. Soft cluster assignments mean some corpus L2 nodes may have a doc-root as child via multiple parent paths.
- **`POST /corpus/raptor/rebuild`** — explicit trigger endpoint, accepts `{workspace_id}` (or implicit from auth header). Defers a `raptor_build_corpus_file` Procrastinate task. Returns `{job_id, status: 'building'}`. Wave A: open (no admin gating); admin protection lands at Phase 9. Not auto-triggered on file uploads — at 100K-doc scale you batch-rebuild on a cadence, not per-upload.
- **`GET /corpus/raptor`** — optional read-side for the corpus tree (level / parent / children). Phase 4 retrieval reads it directly via SQL.
- **Worker stage `raptor_build_corpus_impl(workspace_id)`** — new task, NOT chained from any file-level event. Idempotent: re-run drops old `scope='corpus'` rows for the workspace + rebuilds atomically (whole-tree-or-nothing).
- **New deps:** `umap-learn` (~20 MB), `pynndescent` (UMAP dep).

**Out of scope (Phase 3e):**
- HNSW + BM25 indexes on `raptor_nodes` (Phase 4 — same as 3d).
- Tree-aware retrieval queries (Phase 4).
- Admin gating on `/corpus/raptor/rebuild` (Phase 9 — Wave A ships open per user direction).
- Per-workspace clustering hyperparameter overrides (env-only in Wave A).
- Auto-rebuild on file upload (intentional — at 100K scale this would melt the worker pool).

**Retrieval graceful degradation:** if no corpus tree exists for a workspace, Phase 4 retrieval falls back to per-doc + chunk-level search. Corpus tree is additive, never blocking.

**Estimated wall-clock:** ~4-6 hr G1+G2+G3+G4+G5 — smaller than 3d because schema is unchanged, infrastructure (Summarizer/Embedder factories) is reused as-is. Most of the cost is the UMAP+GMM clustering path + the new endpoint pair.
