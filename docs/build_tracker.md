...
**Now:** Phase 2c ✅ shipped — Gemini OCR + strategy-driven dispatch + per-call override + quality escalation + provenance JSON. 258/258 pytest. verify_phase_2c.sh 15/15. Cross-phase sweep: 9/10 first-pass + 1 transient retry-passed (host memory pressure on the 10th sequential stack triggered Docling first-run flakiness — fixed by bumping the polling buffer 180→240 iters + worker-log capture on failure). Branch `phase-3/chunking-raptor` now carries 5 commit-sets (3a/3b/3c/3b-bis/2c) ready to merge or extend with 3d.
**Next:** Phase 3d — RAPTOR tree build (clustering + Gemini-Flash summarization of L1 contextual chunks → tree nodes). Opens on same branch.
**Blocked on:** nothing.
