...
**Now:** 🎉 **Wave A end-to-end stack shipped.** Ingestion (Phases 2a–7), retrieval (Phases 8a–8h: 6-channel + RRF + Cohere rerank + CRAG + Astute + faithfulness + 13 planner modes), SSE audit (Phase 9), and **9 of 10 UI surfaces** (`/chat`, `/upload`, `/explore` + `/explore/entity/[id]`, `/schema-studio`, `/dashboard`, `/files/[id]`, `/audit`, `/settings`, `/playground`) all ✅ on `main` as of PR #40. `/extraction-studio` ships as a Wave-C roadmap page. Phase 11 demo-corpus loader (`demo-corpus/{build.py,ingest.py}`) + Phase 12 eval runner (`src/kb/eval/{loader,runner,scorer,cli}.py` + `golden_questions.yaml` 45 Qs) both ✅. All 9 tier-1 gap designs from `gaps_design.md` ship as code (Q-mode planner + executor, conflicts, doc chains, corrections, polymorphic citations, vocabulary expansion, lineage ltree, chat memory + anaphora resolver, layered config). Hash-chained `audit_log` + integrity walker live.
**Next:** **Wave-A close-out** — wire RAGAS + HHEM scoring into the eval runner + expose `POST /eval/run` (currently stubbed in the Playground Eval tab as a roadmap placeholder). After that: Wave B competitive commitments (B1 batch query, B2 deep_research agentic loop, B3 DSPy prompt optimization, B4 multi-agent Q-mode decomposition) + Phase 13 NotebookLM-style artifacts. See `docs/competitive_audit.md` §5 for the 4 Wave B commitments.
**Blocked on:** nothing.
