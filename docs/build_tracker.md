...
**Now:** Phase 3e G1 ✅ + G2 ✅ + G3 🟡 OPEN — corpus-level RAPTOR. G2 added `## 6. Phase 3e — Corpus RAPTOR` to `api_contracts.md`: model invariants (§6.1, 7 items), shared schema with per-doc (§6.2), `POST /corpus/raptor/rebuild` endpoint with 202 + `400 corpus-rebuild-no-input` + `503 corpus-rebuild-in-flight` errors (§6.3), out-of-scope deferrals (§6.4). Renumbered old §6→§7 (placeholders) and §7→§8 (changelog). G3 opens. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3e G3 (spec + ~11 red skeletons across `test_raptor_corpus_unit.py` + `test_raptor_corpus_worker.py` + `test_corpus_api.py`) → G4 (code: umap-learn dep + `kb/raptor/corpus.py` + worker task + endpoint router) → G5 (verify_phase_3e.sh + 12-script cross-phase sweep). After 3e, Wave A ingestion + tree-building is fully complete; retrieval (Phase 4 HNSW + BM25 + tree-aware query) opens.
**Blocked on:** nothing.
...
| **3e** | RAPTOR tree build, **corpus-level** — cluster doc-roots across workspace → summarize themes → write `scope='corpus'` rows. Explicit `POST /corpus/raptor/rebuild` trigger (not auto). UMAP+GMM swap-in for the N=100K case where AC is infeasible. | ✅ | ✅ | 🟡 | ⬜ | ⬜ | G2 ✅ 2026-05-24: new `## 6. Phase 3e — Corpus RAPTOR` section in `api_contracts.md` documenting model invariants (§6.1), shared schema with per-doc (§6.2), `POST /corpus/raptor/rebuild` 202 endpoint with `400 corpus-rebuild-no-input` + `503 corpus-rebuild-in-flight` errors (§6.3), and out-of-scope deferrals (§6.4). Renumbered old §6→§7, §7→§8. G3 opens. |
...
### 5.10.1 Phase 3e plan — RAPTOR tree build, corpus-level (G1 ✅ + G2 ✅ + G3 🟡 OPEN)
