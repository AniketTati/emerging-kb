...
**Now:** Phase 3d G1 ✅ + G2 🟡 OPEN — RAPTOR tree build, per-doc. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3d G2 (`api_contracts.md` §5.2 lifecycle enum delta — adds `raptor_building`, reframes `ready`) → G3 (red skeletons) → G4 (0012 migration + raptor + summarization + worker) → G5 (verify_phase_3d.sh + 11-script cross-phase sweep). Then Phase 3e at §5.10.1 (corpus-level, ~4-6 hr, reuses 3d tables via the forward-compat columns).
**Blocked on:** nothing.
...
| **3d** | RAPTOR tree build, **per-doc** — recursive cluster→summarize→re-embed → `raptor_nodes` (L2..6) + `raptor_edges` (discriminated child FK); intermediate lifecycle state `raptor_building` between `embedded` → `ready` | ✅ | 🟡 | ⬜ | ⬜ | ⬜ | G1 ✅ signed off 2026-05-24 post-deliberation. 16 decisions at §5.10. AC(cosine) for per-doc N≤100; UMAP+GMM deferred to 3e where it matters. Discriminated edge FK (L2→contextual_chunks, L3+→self-FK) saves 30 GB at 100K-doc scale. L1 leaves stay in contextual_chunks (no denormalization). Forward-compat `scope` enum + nullable `file_id` locked NOW so 3e needs no migration. G2 opens. |
...
### 5.10 Phase 3d plan — RAPTOR tree build, per-doc (G1 ✅ + G2 🟡 OPEN)
