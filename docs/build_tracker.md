...
**Next:** **Phase 8f ✅ FULLY GREEN — Wave A query stack complete.** Orchestrator + `POST /search` + `POST /chat` + `query_log` audit shipped 2026-05-25 on `phase-8f/orchestrator` (stacked on `phase-8e/generate`). **518/518 pytest in 96s** · `verify_phase_8f.sh` 13/13 standalone. Module `src/kb/query/orchestrator.py` stitches 8a (rewriter, 4-rewrite fan-out) → 8b (6-channel × 4 rewrites = 24 lists → RRF top-30) → 8c (rerank top-10) → 8d (CRAG gate) → 8e (Astute generation, force-refused when CRAG below threshold). `src/kb/api/query.py` mounts `POST /search` (read-only inspector) + `POST /chat` (full pipeline + Idempotency-Key replay). Migration `0019_query_log.sql` adds workspace-scoped + RLS-forced + kb_app-SELECT-INSERT-only audit table per architecture §6. 17 decisions locked at §5.15.6, contracts at api_contracts.md §7 (7 invariants + 5 sub-sections). 27 new tests: 13 orchestrator unit (mocked components — 4-rewrite fan-out · 24-list RRF · top-10 rerank cap · CRAG-after-rerank · force-refuse-on-low-CRAG · empty-corpus refusal envelope · factory-via-env) + 14 HTTP testcontainers (200-with-envelope · 400-on-mode≠H · 200-not-4xx-on-empty-corpus refusal · query_log audit row per call · Idempotency-Key replay returns cached query_id · workspace RLS isolates query_log · query_log kb_app immutability via GRANT). Forward-compat: 5 prior 8x verify scripts widened to allow `kb/api/query.py` past the no-leak grep (8f is the legitimate kb.query→kb.api boundary per architecture). **Wave A query pipeline now end-to-end exposed.** Phase 9 opens next (SSE + `/audit` + lifecycle visibility), then 10a/10b (Next.js Upload + Chat UI).
**Blocked on:** nothing.
