...
**Now:** Phase 2a G4 — building (DDL + Parser Protocol + Docling + worker task + 5 endpoints + main wiring + tiny.pdf fixture). Branch: `phase-2a/parse-scaffold`.
**Next:** Phase 2a G5 — `verify_phase_2a.sh` + run all 5 verify scripts + end-of-phase sweep.
**Blocked on:** nothing.
...
| **2a** | Parse layer — **scaffold + Docling**: `files` + `file_lifecycle` + `raw_pages` + `parse_artifacts` tables; Procrastinate `parse_file` task; MIME-based dispatcher; Docling (digital PDF) parser; admin `POST /files` upload endpoint | ✅ | ✅ | ✅ | 🟡 | ⬜ | G1+G2+G3 ✅ signed off 2026-05-23. 28 new tests collected (files_crud 10 + parse_dispatch 5 + parse_pdf_docling 3 + raw_pages 5 + files_lifecycle 5); pytest --collect-only confirms 170 total. G4 building now. |
