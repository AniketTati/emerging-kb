...
**Now:** Phase 3c — G1–G4 all green; G5 script authored (`verify_phase_3c.sh`, 15 checks) but Docker-stack run blocked on host disk pressure (88% full, ~2 GB free → OrbStack daemon stopped). pytest path is authoritative + green: 232/232. Tracker reflects G5 🟡 until disk reclaim + verify run.
**Next:** User: free ~5-10 GB host disk → re-run `./scripts/verify_phase_3c.sh` + cross-phase sweep to fully close G5; then Phase 3d (RAPTOR tree build) opens on same branch.
**Blocked on:** **host disk space** — Docker daemon needs to be reachable for the Docker-stack verify scripts. Pytest suite remains green and authoritative.
...
| **3c** | Embedding — Gemini Embedding 001 on contextual chunks → `chunk_embeddings` (`halfvec(3072)`); worker stage `embed_file`; new lifecycle state `embedded` | ✅ | ✅ | ✅ | ✅ | 🟡 | First embedding call; gated on `KB_GEMINI_API_KEY` with DeterministicMockEmbedder for CI. 13/13 new tests green; suite 232/232. **G5 script authored** (verify_phase_3c.sh, 15 checks); Docker-stack run blocked on host disk pressure (88% full, 2 GB free → OrbStack daemon down). pytest path is authoritative + green; Docker-stack run will close G5 after disk reclaim. |
...
| 3c | [scripts/verify_phase_3c.sh](../scripts/verify_phase_3c.sh) | 2026-05-23 (authored; Docker-stack run deferred) | 🟡 Script authored (15 checks: compose smoke + 5 DDL assertions including `halfvec` column type + E2E PDF parse-chunk-contextualize-embed + model_id='mock-deterministic-v1' check + lifecycle progression + idempotent re-defer + Phase-3c pytest 13); Docker-stack run blocked on host disk pressure. pytest 232/232 confirmed via testcontainers (primary validation path). |
| ... | | | |
