...
**Next:** **Phase 8d ✅ FULLY GREEN** — CRAG (Corrective RAG) relevance gate shipped 2026-05-25 on `phase-8d/crag` (stacked on `phase-8c/rerank`). **472/472 pytest in 90s** · `verify_phase_8d.sh` 10/10 standalone. Module `src/kb/query/crag.py` (~210 LOC) with `CragGate` Protocol + 2-impl factory (`GeminiCragGate` + `IdentityCragGate`) per CRAG paper (Yan et al. 2024). `CRAG_THRESHOLD = 0.5`. 10 decisions locked at §5.15.4 including #10 anthropic→Identity Wave-A defer. 16 new unit tests (parser fail-safes + Identity always-1.0 + Gemini empty→0.0 + factory matrix + mocked Gemini path with top-3 capping). No migration; no API contract change. **Phase 8e opens next** — Astute generation (Gemini answer with chunk + entity + atomic-unit citations + refusal mode when CRAG score < threshold). Then 8f (orchestrator + `POST /search` + `POST /chat` + `query_log` migration), 9 (SSE + audit), 10a/10b (Next.js UI). Stacked-branch PR strategy: each phase off the previous unmerged phase.
**Blocked on:** nothing.
