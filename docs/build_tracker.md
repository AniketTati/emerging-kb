...
**Now:** Phase 3e G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN — corpus-level RAPTOR. Code landed (2 new modules + 4 mutations). 286/286 pytest in 81s. G5: new `verify_phase_3e.sh` (12 checks: compose smoke + umap-learn import probe + POST 5 PDFs through to ready + POST /corpus/raptor/rebuild → 202 + wait for corpus job to complete + scope='corpus' nodes exist + cross-scope edges + atomic re-rebuild + Phase-3e pytest 11) + 12-script cross-phase sweep. Plan at §5.10, **16 decisions** (revised post-deliberation). Three deliberation flips: (1) discriminated edge FK + L1 stays in contextual_chunks — saves 30 GB at 100K-doc scale; (2) `raptor_building` intermediate lifecycle state — observability for the multi-stage build; (3) sharpened Identity Summarizer framing — no-key smoke path only, not CI semantic coverage. Forward-compat: `raptor_nodes.scope` enum + nullable `file_id` locked NOW so Phase 3e (corpus-level RAPTOR) needs no migration. Bumped `MAX_LEVELS` 4→6 to cover corpus-tree depth (`log₈(100K)≈5.5`).
**Next:** Phase 3e G5 (new `verify_phase_3e.sh` + 12-script cross-phase sweep) → push. After 3e, **Wave A ingestion + tree-building is fully complete**; retrieval (Phase 4 HNSW + BM25 + tree-aware query) opens.
**Blocked on:** nothing.
...
| **3e** | RAPTOR tree build, **corpus-level** — cluster doc-roots across workspace → summarize themes → write `scope='corpus'` rows. Explicit `POST /corpus/raptor/rebuild` trigger (not auto). UMAP+GMM swap-in for the N=100K case where AC is infeasible. | ✅ | ✅ | ✅ | ✅ | 🟡 | G4 ✅ 2026-05-24: 2 new modules + 4 mutations. `src/kb/raptor/corpus.py` (UMAP+GaussianMixture clustering with `random_state` determinism + soft-falls-back to GMM-only when N is too small for UMAP + `read_doc_roots_for_workspace` heterogeneous reader returning `(id, text, vec, kind)` where kind ∈ {'node','chunk'} + `delete_corpus_rows_for_workspace` for atomic rebuild). `src/kb/api/corpus.py` (POST /corpus/raptor/rebuild with 2 pre-flight checks). Mutations: `src/kb/workers/tasks.py` adds `raptor_build_corpus_impl` + `raptor_build_corpus` Procrastinate task (atomic DELETE-old + INSERT-new in one tx + handles heterogeneous L2 edges via discriminated FK), `src/kb/api/main.py` mounts corpus router + 2 new exception handlers, `src/kb/api/errors.py` adds `CorpusRebuildNoInputError` + `CorpusRebuildInFlightError`, `.env.example` adds 3 UMAP/GMM tuning vars. New deps: umap-learn>=0.5.12 + pynndescent + numba + llvmlite. Two in-G4 fixes: (1) initial `raptor_build_corpus_impl` didn't import `math` + `os` inside function body; (2) corpus worker tests needed `KB_DATABASE_URL=db_url_superuser` + `get_settings.cache_clear()` since they don't use the `client` fixture that transitively sets it. Suite: 286/286 in 81s (275 prior + 11 new). G5 opens. |
...
### 5.10.1 Phase 3e plan — RAPTOR tree build, corpus-level (G1 ✅ + G2 ✅ + G3 ✅ + G4 ✅ + G5 🟡 OPEN)
