...
**Next:** **Phase 4 G5 opens** (verify). G1 ✅ + G3 ✅ + G4 ✅ signed off. **296/296 pytest in 89.84s.** G4 commits: (a) `migrations/runner.py` — `@no-transaction` pragma support (Postgres rejects `CREATE INDEX CONCURRENTLY` inside transaction blocks; pragma honored by `_apply_one` to run statements under autocommit; same path will be used by Phase 14 HippoRAG graph indexes). (b) `migrations/sql/0013_indexes.sql` — 4 CONCURRENTLY indexes (HNSW with halfvec_cosine_ops/m=16/ef_construction=200 + BM25 with pg_search defaults). (c) `src/kb/retrieval/{__init__,smoke}.py` — `bm25_smoke` + `dense_smoke` helpers (internal, NOT on any router). (d) `scripts/reindex_weekly.sh` — cron stub. **3 planner-usage pytest tests dropped** — at fixture scale (~200 rows) the planner correctly prefers btree index-scan + sort over HNSW (HNSW only wins >5K rows); moved to G5 where verify_phase_4.sh exercises EXPLAIN against realistic seed data + ANALYZE. G5 deliverable: `scripts/verify_phase_4.sh` (DDL existence + planner usage + smoke helper E2E + 5-doc seed via real pipeline) + cross-phase sweep across all 13 verify scripts via `scripts/verify_sweep.sh`.
**Blocked on:** nothing.
...
| **4** | Indexing: pgvector HNSW + pg_search BM25 on all RAPTOR levels | ✅ | — | ✅ | ✅ | ⬜ | G1 ✅ + G3 ✅ + G4 ✅ signed off 2026-05-25 (§5.11, 16 decisions). G2 was a no-op per decision #16. G4 lands `0013_indexes.sql` (4 CONCURRENTLY indexes — needed `@no-transaction` pragma added to `migrations/runner.py`) + `src/kb/retrieval/{__init__,smoke}.py` + `scripts/reindex_weekly.sh` stub. 3 planner-usage tests moved from pytest to G5 verify_phase_4.sh (planner correctly picks btree at fixture scale; HNSW only wins >5K rows). **296/296 pytest in 89.84s.** Branch `phase-4/retrieval`. |
...
### 5.11 Phase 4 plan — Indexing (HNSW + BM25 on all RAPTOR levels) (G1 ✅ + G2 — + G3 ✅ + G4 ✅ SIGNED OFF · G5 🟡 OPEN)

> **Status:** G1 ✅ + G3 ✅ + G4 ✅ signed off 2026-05-25 by Aniket. G2 was a no-op per decision #16. G5 opens: `scripts/verify_phase_4.sh` + cross-phase sweep across 13 scripts. Branch: `phase-4/retrieval` off main at `fff1e3f`. First Wave A phase past ingestion. **Indexing-only** — no HTTP retrieval surface, no orchestration, no rerank. That's all Phase 8 ("Query planner + rewriting + parallel retrieval + RRF + rerank + CRAG + Astute" per architecture §12 line 1167–1169). Phase 4 lays the index foundation that Phase 8 will consume; the 7 of 10 channels that need L2/L2b/L3 atomic-unit + mention + entity artifacts (per README §3 pillar 3) are gated on Phase 5–7 first.
